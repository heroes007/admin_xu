...
<script>
  ...
  import {ADD_TEACHER, ADD_CERTIFICATE} from '../views/dialogs/types'
  ...
  export default {
    ...
    methods: {
      deleteHandler() {
        this.$Modal.confirm({
          title: '提示',
          content: this.type ? '是否确定删除该证书？' : '是否确定删除该讲师？',
          onOk: () => {
            this.type ? this.$store.dispatch('delete_certificate', {id: this.tid}) : this.$store.dispatch('delete_teacher', {id: this.tid})
          }
        });
      },
      editTeacher() {
