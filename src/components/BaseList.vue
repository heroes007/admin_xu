<template>
    <div class='base-list-container'>
        ...
        <el-table @selection-change='selectionChangeHandler' :row-class-name="rowClassName"
            ...
            <el-table-column type="selection" width="55" v-for='item in selectionHeader'
                             :key="item.id"></el-table-column>
            <el-table-column type="index" :label="getHeaderLabel(item)" width="65" v-for='item in sortHeader'
                             :key="item.id"></el-table-column>
            <el-table-column :prop="item.prop" :label="getHeaderLabel(item)" :width="item.width"
                ...
                <template slot-scope="scope">
                    ...
                    <a target="_blank" class="a-link" :href="showPropValue(item.prop,scope.row)"
                       v-if="!item.mixColumn && !item.isBtn && !item.limit && item.isLink && !item.useCombo && !item.useTimePicker &&!item.useMark">
                        {{showPropValue(item.prop,scope.row)}}
                    </a>
            ...
            <el-table-column :label="getHeaderLabel(item)" :width="item.width" header-align='left' align='left'
                             :show-overflow-tooltip="true" v-for='item in btnHeader'
                             :key="item.id">
                <template slot-scope="scope">
                    <div class='handle-component' v-for='btn in item.groupBtn' :key="btn.id"
                        ...
                        <el-checkbox v-model='scope.row[btn.switchKey]'
                                     @change='changeSwitchValue(scope.row,btn.switchKey,btn.actionName,btn.param)'
            ...
            <el-table-column type='expand' :width="item.width" header-align='left' align='left'
                             v-for='item in listExpandHeader' :key="item.id">
                <template slot-scope="scope">
                    <baseList class='child-list data-list' :table-data='scope.row.childData'
                              :header-data='item.childHeader' :is-stripe='false'
                              :parent-data='scope.row' :column-formatter='item.listColumnFormatter'
                              @childBtnClick='childBtnClickHandler'>
                    </baseList>
                </template>
            </el-table-column>
        </el-table>
