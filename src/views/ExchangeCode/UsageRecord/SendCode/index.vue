<template>
    <Modal v-model="showContent" :title="title" :width="654" @on-cancel="closeModal"  :mask-closable=false :footer-hide="true" >
       <div class="exchange-content-title">
            <Input class="exchange-input" v-model="value" @on-change="changeVal" placeholder="搜索用户名/姓名/手机号">
              <Icon type="md-search" slot="prefix" />
            </Input>
       </div>
       <div class="exchange-main">
         <Tables :column="columns1" :table-data="codeList"  @radio-change="radioChange"/>
       </div>
        <div class="foot-btn">
             <Button class="btn-orange" type="primary" @click="handleSubmit">保存</Button>
        </div>
    </Modal>
</template>

<script>
 import  Tables from '../../../../components/tables.vue'
  export default {
    components: { Tables },
    props: {
        title:{ type: String },
        showModal: {
            type: Boolean,
            default: false
        },
        list: {
          type: Array,
          default: []
        }
    },
    watch:{
        showModal(_new){
            this.showContent = _new
        }
    },
    data(){ 
        return {
            value:'',
            showContent: false,
            radioData: null,
            selectChecked: null,
                 columns1: [
            {
                title: '用户名',
                key: 'realname',
            },{
                title: '姓名',
                key: 'name',
            },
            {
                title: '手机号',
                key: 'num',
            },
             {
                title: '科室',
                key: 'already',
            },
            {
                title: '班级',
                key: 'classs',
            },
            {
                title: "选择",
                slot: 'radio-item',
                key: 'state',
            },
            ],
            codeList: [
            {
                "id": 13186,
                "realname": "ur201232",
                "name": "王晓东",
                "product": "浙江全科医生培训",
                "num": '18729816382',
                "already": '呼吸内科',
                classs:'住培一年级',
                state: false
            },
             {
                "id": 13188,
                "realname": "ur201232",
                "name": "王晓东",
                "product": "浙江全科医生培训",
                "num": '18729816382',
                "already": '呼吸内科',
                classs:'住培一年级',
                state: false
            },
        ],
        }
    },
    methods:{
        closeModal(){
            this.showContent = false
            this.$emit('close')
        },
        radioChange(v){
          this.radioData = v
        },
        changeVal(v){
            console.log(v)
        },
        handlieList(){
            this.list.map((t) => {
                t.show = false
            })
        },
        handleSubmit(){
           if(this.radioData){
                console.log(this.radioData)
                this.closeModal()
           }else  this.$Message.warning('请选择用户');
        }
    },
    mounted() {
      this.handlieList()
    }
  }
</script>
<style lang='less' scoped>
.exchange-content-title{
    width: 100%;
    height: 60px;
    background: #F0F0F7;
    .exchange-input{
        margin-top: 12px;
        margin-left: 30px;
        width: 350px;
        height: 36px;
        border-radius: 18px;
        /deep/ .ivu-input{
            border-radius: 18px !important;
        }
        /deep/ .ivu-input-prefix{
            margin-left: 10px;
        }
        /deep/ .ivu-icon-md-search{
            font-size: 20px;
        }
    }
}
/deep/ .ivu-modal-body{ padding: 0 !important; padding-bottom: 40px;}
.exchange-main{
    height: 574px;
    overflow: hidden;
    overflow-y: auto;
    .exchange-list{
    height: 62px;
    border-bottom: 1px solid #E4E7F5;
    position: relative;
    line-height: 62px;
    .exchange-item{
        font-family: PingFangSC-Regular;
        font-size: 16px;
        color: #474C63;
        padding-left: 40px;
    }
    .exchange-img{
        position: absolute;
        top: 22px;
        right: 40px;
        width: 19px;
        height: 17px;
    }
}
}
.foot-btn{ 
    display: flex; justify-content: center;
    padding-top: 30px;
    height: 106px;
    .btn-orange{
       width: 200px;
       height: 36px;
    }
}

</style>
