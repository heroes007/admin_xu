        ...
        <base-input @closedialog="handleClose">
            <Row slot="body">
                <Row class='curriculum-list' v-if='nextStep == 1'>
                    ...
                    <Row>
                        <data-list class='data-list light-header' @changeSelect='changeRowSelectHandler'
                                   :table-data='curriculumList' :header-data='dataHeader'></data-list>
                    </Row>
                </Row>
                <Row class="body-top">
                    <Form ref="form" :model="form" :label-width="fromLabelWidth" class="add-task-form">
                        ...
                        <FormItem v-show="nextStep == 0" label="荣誉证书">
                            <Select v-model="form.certificate" multiple>
                                <Option v-for="(item, index) in certificate" :key="item.id" :value="item.id"
                        ...
                        <FormItem label="展示图片" v-if="nextStep == 0">
                            <upload-panel ref="upload_panel" :resourse="form.img_url" :upload-config="uploadConfig"
                                          @uploadcomplete="uploadCompleteHandler1">
                        ...
                        <FormItem label="展示视频" v-if="nextStep == 0 && form.displayVideo">
                            <upload-panel :resourse='form.video_url' @uploadcomplete='uploadCompleteHandler2'
                                          :upload-config='uploaderConfig2'>
                                <span slot="file-require">只能上传 MP4/MOV/AVI 文件，且不超过2M</span>
                            </upload-panel>
                        </FormItem>
                        <FormItem v-show="nextStep == 0" label="产品简介">
                            <Input type="textarea" :rows="6" placeholder="请输入产品简介"
                                   v-model="form.short_description"></Input>
                        </FormItem>
                        ...
                        <FormItem class="btns">
                            <Button type='text' v-show='nextStep == 2 || nextStep == 1' class='btn-pre'
                                    @click='handlePreStep'>上一步
                            </Button>
                            <Button v-show="nextStep == 2" class="btn-orange" @click="handleSubmit('form')">提交</Button>
                            <Button v-show="nextStep == 0 || nextStep == 1" class="btn-orange btn-next"
