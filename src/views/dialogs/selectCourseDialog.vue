<template>
    <Modal :transfer=false title="课程列表" :width="600" :mask-closable="false" :footer-hide=true
           v-model="selectCurriculumDialog" @on-cancel="handleRemoveModal(remove)">
        <base-input @closedialog="handleClose">
            <Row slot="body">
                <Row class="body-top body-list">
                    <data-list class='data-list light-header' @changeSelect='changeRowSelectHandler'
                               :table-data='dataList' :header-data='dataHeader'></data-list>
                </Row>
                <Row class="elRow">
    ...
    methods: {
      ...
      addHandler(formName) {
        ...
        if (result.length === 0) {
          ...
          var vm = this;
          this.add_production_curriculums({
            id: this.payload.productionId, curriculums: result, curriculumData: datas, _fn: function () {
              vm.handleClose();
              vm.showPop('添加成功！');
            }
          })
