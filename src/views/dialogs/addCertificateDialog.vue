<template>
    <Modal :transfer=false title="添加证书" width="600px" v-model="addTeacherDialog" @on-cancel="handleRemoveModal(remove)" size="auto" :mask-closable="false" :footer-hide="true">
            ...
            <Row slot="body">
                <Row class="body-top" v-if="true">
                    <Form ref="form" :model="form" :label-width="80" class="add-teacher-form">
                        ...
                        <FormItem label="证书描述">
                            <Input type="textarea" style="resize: none" :autosize="{ minRows: 6, maxRows: 6}" placeholder="请对证书进行描述" v-model="form.detail"></Input>
                        </FormItem>
                        <FormItem label="证书照片" class="upload-form1">
...
<script>
  ...
  import {get_certificate_detail} from '../../api/modules/tools_product'
  ...
  export default {
    ...
    methods: {
      saveHandler() {
        if(this.payload) this.$store.dispatch('edit_certificate',this.form);
        else  this.$store.dispatch('add_certificate',this.form);
      },
      handleClose() {
    ...
    mounted(){
      ...
      if(this.payload){
        ...
        //   }
        // })
        get_certificate_detail(this.payload).then(res => {
