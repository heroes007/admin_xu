...
import { ADD_REMEED_CODE, REMEED_CODE_DETAIL } from '../dialogs/types'
import { doTimeFormat, exportCsv } from '../../components/Util'
import { Config } from '../../config/base'
import { query_code, get_list_by_group } from '../../api/modules/tools_redeem_code'
export default {
    mixins: [Dialog],
    components: { 'data-list': BaseList },
    data() {
        return {
            searchData:'',
            ...
            },
            {
                ...
                groupBtn: [{
                    ...
                    disabledFuc:(function(row){
                        if(new Date(row.end_time).getTime() < new Date().getTime()) return true;
                        return false;
                    })
                },{
                  text: '导出',
    ...
    methods: {
        ...mapActions([ 'get_code_group_list', 'get_production_list' ]),
            ...
            get_list_by_group(row.id,0,100000).then(res => {
                if(res.data.res_code === 1){
                    ...
                    for(var i=0;i<data.length;i++){
                        if(data[i].state === -1)  data[i].useTime = doTimeFormat(data[i].use_time);
                        else data[i].useTime = '';
        ...
        getCodeStateStr(state){
            if(state === 0) return '不可用';
            else if(state === 1) return '未兑换';
            else if(state === -1) return '已兑换';
            else if(state === -2) return '已作废';
        },
        searchHandler(){
            if(!this.searchData){
                this.$Modal.info({
    ...
    mounted() {
        ...
        this.get_production_list({ project_id: this.projectId, page_index: 0, page_size: 999, state: [0, 1] });
    }
}
</script>
