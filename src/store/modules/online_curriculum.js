...
const actions = {
    ...
    get_online_curriculum_chapter_list({ commit }, params) {
        ...
        get_catalog(params.curriculum_id).then(res => {
           if(res.data.res_code === 1)
           {
               commit(types.ONLINE_CURRICULUM_CHAPTER_LOADED,{result:res.data.msg,curriculum_id:params.curriculum_id});
           }
        });
    },
    ...
    save_online_curriculum_chapter_orderby( {commit} ,param) {
        ...
        for(var i=0;i<state.online_curriculum_list.length;i++)
        {
            if(state.online_curriculum_list[i].curriculum_id == curriculum_id)
            {
                ...
                {
                    params.push({catalog_id: state.online_curriculum_list[i].chapterList[j].catalog_id, orderby: state.online_curriculum_list[i].chapterList[j].orderby});
                }
                break;
            }
        }
    ...
    edit_online_curriculum_video( {commit}, params) {
        commit(types.ONLINE_CURRICULUM_CHAPTER_SHOW_LOADING);
        update_video(params.video_id,params).then(res => {
            if(res.data.res_code === 1)
            {
